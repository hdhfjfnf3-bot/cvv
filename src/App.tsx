...
export default function App() {
    ...
    return (
        <div dir="rtl" className="min-h-screen overflow-x-hidden bg-[#05060A] text-white">
            ...
            <div className="relative z-10 mx-auto min-h-screen w-full max-w-[1280px] px-0 py-0 xl:px-6 xl:py-6">
                <div className="min-h-screen overflow-hidden rounded-none border-0 bg-[#0B0B0F]/90 shadow-none xl:min-h-[calc(100vh-3rem)] xl:rounded-[40px] xl:border xl:border-white/10 xl:bg-[#0B0B0F]/76 xl:shadow-[0_0_0_1px_rgba(255,255,255,0.03),0_30px_80px_rgba(0,0,0,0.5),0_0_60px_rgba(0,209,255,0.08)]">
                    ...
                    <main className="relative z-10 mx-auto flex max-w-6xl flex-col gap-6 px-4 pb-16 pt-5 sm:gap-10 sm:px-6 sm:pt-8">
                            ...
                            >
                                <GlassCard className="h-full p-5 sm:p-6">
                                    ...
                                    <div className="mt-6 rounded-[20px] border border-white/10 bg-white/5 p-4">
                                        <p className="text-sm text-white/55">آخر الرسائل المستلمة</p>
                                        <div className="mt-4 space-y-3">
                                            {messages.slice(0, 2).map((item) => (
                                                <div key={item.id} className="rounded-2xl border border-white/10 bg-black/20 p-3">
                                                    <p className="text-sm font-medium text-white">{item.name}</p>
                                                    <p className="mt-1 text-xs leading-6 text-white/55">{item.message}</p>
                                                </div>
                                            ))}
                                            {!messages.length && !loading ? <p className="text-sm text-white/45">كن أول من يرسل رسالة الآن.</p> : null}
                                        </div>
                                    </div>
                                </GlassCard>
                            </motion.div>
                        ...
                        <footer className="pb-4 pt-2 text-center text-xs text-white/35 sm:text-sm">
                            © 2025 ميديا باير برو — مصمم لتحقيق التحويل
                        </footer>
                    </main>
                </div>
            </div>
